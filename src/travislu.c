...
void travislu_terminate(unsigned long *time_info, unsigned long *mem_info, int count)
{
	...
	fp = fopen("/home/travis/mosquitto/expi/mem_info.csv", "w");
	fp_sub = fopen("/home/travis/mosquitto/expi/sub_queue.csv", "w");
	fp_db = fopen("/home/travis/mosquitto/expi/db_queue.csv", "w");
...
void travislu_get_sub_queue_time(struct mosquitto_client_msg *msg)
{
    ...
    temp = strstr((msg->store)->topic, "test");
    if (!temp)
	return;
